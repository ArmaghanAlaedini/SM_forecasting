...
def keep_pixel_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df[cfg.PIXEL_OUTPUT_COLUMNS].copy()
...
def standardize_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Latitude/longitude remain float64 for stable coordinate joins.
    Weather variables are float32.
    """
    string_cols = [
        "valid_date",
        "valid_time",
...
def build_regular_latlon_cells(
    sub: pd.DataFrame,
    model: str,
    product: str,
) -> gpd.GeoDataFrame:
    ...
    return clip_cells_to_iowa(cells)


def build_projected_square_cells(
    sub: pd.DataFrame,
    model: str,
    product: str,
) -> gpd.GeoDataFrame:
...
# ============================================================
# 11. Daily Iowa average and existing-pixel geometry rebuild
# ============================================================
...
def process_model_year(
    ...
    If the pixel file already exists and RESUME_SKIP_EXISTING=True,
    skip extraction but optionally rebuild geometry from the existing pixel file.

    If the pixel file does not exist, run the full extraction.
    """
    ...
    # ------------------------------------------------------------
    # Case 1: pixel file already exists.
    # Skip extraction, but rebuild geometry if requested.
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Case 2: pixel file does not exist.
    # Run full extraction.
    # ------------------------------------------------------------
