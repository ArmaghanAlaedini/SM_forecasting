...
-- updated to include pred_regression_kriging as a meta-model feature --
"""
...
def plot_scatter(y_true, y_pred, path: Path) -> None:
...
def main() -> None:
    print("10g: Train stacking meta-model (with regression_kriging feature)")
    print("=" * 70)
    print(f"Meta table:   {META_TABLE_PATH}")
    print(f"Output model: {OUT_MODEL_PATH}")
    print(f"Base pred cols: {BASE_PRED_COLS}")
    print("=" * 70)
    ...
    print(f"\nMeta-model features ({len(feature_cols)}):")
    for c in feature_cols:
        n_valid = meta[c].notna().sum()
        print(f"  {c:<48}  {n_valid:>8,}  ({100*n_valid/len(meta):.1f}%)")
    ...
    X_tr, X_ts, y_tr, y_ts = train_test_split(X_all, y_all, test_size=DIAGNOSTIC_TEST_FRACTION, random_state=RANDOM_STATE)
    ...
    print(f"\nRidgeCV alpha: {diag_pipe.named_steps['ridge'].alpha_}")
    plot_scatter(y_ts, y_pred_diag, OUT_SCATTER_PATH)

    print("\nRe-training on all rows...")
    ...
    report = [
        "Stacking meta-model training report", "=" * 50,
        f"Meta table rows:  {len(meta):,}",
        f"Feature columns:  {len(feature_cols)}",
        f"Base pred cols:   {BASE_PRED_COLS}",
        f"RidgeCV alpha:    {final_pipe.named_steps['ridge'].alpha_}",
        f"Intercept:        {final_pipe.named_steps['ridge'].intercept_:.6f}",
        "", "Diagnostic metrics (held-out 20%)", "-" * 50,
    ] + [f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}" for k, v in diag_metrics.items()] + [
        "", "Coefficients (by abs value)", "-" * 50,
