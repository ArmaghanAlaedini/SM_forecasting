...
#
# Independent 2025 test of the selected GI methods.  The script reads the exact
# same test_holdouts_2025.csv used by 10d; it does not create its own random or
# spatial gaps.  The method implementations/settings are shared with 10b/11b.
...
TEST_DIR <- file.path(
  PROJECT_ROOT,
  "src/data/processed/smap_gap_filling/06_selected_methods_test"
)
HOLDOUT_PATH <- file.path(TEST_DIR, "holdouts/test_holdouts_2025.csv")
OUT_DIR <- file.path(TEST_DIR, "interpolation")
dir.create(OUT_DIR, recursive = TRUE, showWarnings = FALSE)

PREDICTION_PATH <- file.path(OUT_DIR, "interpolation_selected_test_predictions.csv")
METRICS_PATH <- file.path(OUT_DIR, "interpolation_selected_test_metrics.csv")
DAILY_METRICS_PATH <- file.path(OUT_DIR, "interpolation_selected_test_daily_metrics.csv")
COVERAGE_PATH <- file.path(OUT_DIR, "interpolation_selected_test_coverage.csv")
...
process_group <- function(group_manifest) {
  ...
  if (nrow(target_dt) != nrow(group_manifest)) {
    ...
    stop(
      "Not all test targets were found in ", path,
...
main <- function() {
  message("10e: Independent 2025 GI test on shared holdouts")
  message(strrep("=", 78))
